...
function seedBlogs(){
	...
	return Blogs.insertMany(seedPost);
}
...
describe('Testing blog API', function(){
	...
	//Insert documents(records) into test data
	beforeEach(function(){
		return seedBlogs();
	});

	//Delete (remove) records from test database
	afterEach(function(){
		return deleteDB();
	})
	//Close open server connection 
	after(function(){
		return closeServer();
	});
	    ...
	    it('should return posts with right fields', function () {
	      ...
	      return chai.request(app)
	        ...
	        .then(function (res) {
	          ...
	          resPost = res.body[0];
	          return Blogs.findById(resPost.id);
	        })
	        .then(post => {
